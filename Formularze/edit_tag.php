...
<form action="view_tag.php?id=<?php echo $row['id']; ?>" method="POST" accept-charset="UTF-8" enctype="application/x-www-form-urlencoded" onsubmit="return ajax_check()">
	...
	<div>
		...
		<input type="text" name="nazwa" id="nazwa" value="<?php echo $row['nazwa']; ?>" size="32" maxlength="32" spellcheck="true" onchange="check_tag()" required="required" />
...
<?php
					bottom(array('https://ajax.googleapis.com/ajax/libs/jquery/1.11.2/jquery.min.js','js/js-webshim/minified/polyfiller.js','js/ask_db.js','js/remaining_char_counter.js','js/tag_form_edit.js'));
				}
				else{
